...
question$startdate=strptime(question$startdate,"%Y-%m-%d")
question$enddate=strptime(question$enddate,"%Y-%m-%d")
...
for (i in 1:nrow(question)){
  ...
  for (j in 1:length(respondent)){
   ...
   if (que_type==0){
      ...
      currcube=employee$cube_id[employee$emp_id==currresp]  
      currcubeemp=employee$emp_id[employee$cube_id==currcube]
      currcubeempnot=employee$emp_id[!(employee$emp_id %in% currcubeemp)]
      if(nrow(wesub)==0){
        target_emp_id_cube=currcubeemp[sample(1:length(currcubeemp),sample(4:7,1),replace = FALSE)]
        target_emp_id_cubenot=currcubeempnot[sample(1:length(currcubeempnot),sample(1:3,1),replace = FALSE)]
        ...
        target_emp_id_cube=currcubeemp[sample(1:length(currcubeemp),sample(1:2,1),replace = FALSE)]
        target_emp_id_cubenot=currcubeempnot[sample(1:length(currcubeempnot),sample(1:2,1),replace = FALSE)]
