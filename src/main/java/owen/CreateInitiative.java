...
public class CreateInitiative {
	...
	public static void main(final String[] args) throws IOException, ParseException {
		...
		List<Employee> employeeList = initiative.getEmployeeSmartList(params);
		ArrayList<String> empIdList = new ArrayList<>();
		for(Employee e : employeeList){
			empIdList.add(e.getId());
		}
		
		initiative.setInitiativeProperties("Initiative30", "Change Process", "01/01/2015", "01/05/2015", "xyz", funcList, zoneList, posList, empIdList);
	 ...
	 */
	public List<Employee> getEmployeeSmartList(Map<String, Object> params) {
		List<Employee> employeeList = new ArrayList<Employee>();
		try (Transaction tx = dch.graphDb.beginTx()) {
			org.apache.log4j.Logger.getLogger(CreateInitiative.class).debug("getEmployeeSmartList method started");
			...
			org.apache.log4j.Logger.getLogger(CreateInitiative.class).debug("query : " + query);
			...
			while (res.hasNext()) {
				...
				e.setId(empId);
				e.setName(resultMap.get("Name").toString());
				e.setScore((Long) resultMap.get("Score"));
				org.apache.log4j.Logger.getLogger(CreateInitiative.class).debug("Employee  : " + e.getId() + "-" + e.getName() + "-" + e.getScore());
				employeeList.add(e);
			}
			tx.success();
			org.apache.log4j.Logger.getLogger(CreateInitiative.class).debug("employeeList : " + employeeList.toString());
			return employeeList;
		}
	}
